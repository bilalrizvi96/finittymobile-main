...
class _RollingNavBarInnerState extends State<_RollingNavBarInner>
  ...
  double get maxWidth =>
      (widget.numChildren - 1) * tabChunkWidth + (tabChunkWidth / 1.75);

  @override
  Widget build(BuildContext context) {
    double Function(double) directionalityCenterXTransform = widget.isLtr
        ? (x) => x + (tabChunkWidth / 1.75) - indicatorRadius
      ...
      children: [
       ...
       Stack(
         ...
         //  Container(color: Colors.white,),
           ActiveIndicator(
             centerX: directionalityCenterXTransform(indicatorX-8),
             centerY: (widget.height / 2.2) - indicatorRadius,
             color: indicatorColor,
             cornerRadius: widget.indicatorCornerRadius,
             height: indicatorRadius * 2.2,
             numSides: widget.indicatorSides,
             rotation: widget.isLtr ? indicatorRotation : indicatorRotation * -1,
             width: indicatorRadius * 2.2,
           ),
           Positioned(
  ...
  Widget _buildNavBarIcon(Indexed indexed) {
    ...
    return _buildNavBarItem(
      Image.asset(
        ...
        height: widget.iconSize[indexed.index],
        width:  widget.iconSize[indexed.index],),
      // Icon(
      //   indexed.value,
      //   color: isActive
      //       ? _getActiveIconColor()
      //       : _getInactiveIconColor(indexed.index),
      //   size: widget.iconSize[indexed.index],
      // ),
      indexed,
    );
  }
...
//  final Widget badge;
  final Widget child;
  final Color color;
  _OptionalBadge({ this.child, this.color, Key key})
      : assert(child != null),
        super(key: key);
